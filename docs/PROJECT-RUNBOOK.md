...
| **Security (repo)** | **`lpnw-login-as.php`**: dev default key + optional **`LPNW_LOGIN_AS_SECRET`** override; **`hash_equals`** for key check. **`lpnw-autologin.php`** still self-deletes after one success. Harden before launch per `docs/DEPLOYMENT.md`. |
