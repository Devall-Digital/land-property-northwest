...
## Tokens and Mautic preview

**Greeting:** use `{contactfield=firstname|there}` so **contact preview** in Mautic shows the right first name from the contact record (same field WordPress syncs on send).

**Filled only when WordPress sends the alert** (API `tokens` payload):

| Token                    | Content                              |
| ------------------------ | ------------------------------------ |
| `{lpnw_alert_count}`     | Number of listings in this send      |
| `{lpnw_properties_html}` | Pre-built HTML summary of properties |
| `{lpnw_postcode_areas}`  | Optional plain lines (per property)  |

Mautic **preview** will still show `{lpnw_alert_count}` and `{lpnw_properties_html}` as literal text until a real send from the site. That is expected.

Add more keys via filter `lpnw_mautic_alert_email_tokens` in WordPress.

## Minimal HTML example

```html
<p>Hi {contactfield=firstname|there},</p>
<p>You have <strong>{lpnw_alert_count}</strong> new match(es) on your <strong>VIP</strong> plan.</p>
