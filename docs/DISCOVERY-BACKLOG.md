...
| 2026-04-03 | **Shipped (Plugin 1.0.9):** Hero **v3** — three **full-width** procedural SVG layers (sky, mid skyline, front skyline + trees/street); window grids; unwraps `lpnw-hero__scene` when wrapping only illustration. **Mautic:** Settings lists **recent email IDs** via API. **Docs:** `mautic-templates-setup.md`, cron 401 note in runbook + owner todo. **Theme 6.4.0:** `.lpnw-hero__parallax` + layer parallax via `--lpnw-parallax-p`. |
...
| 2026-04-02 | **Shipped (Plugin 1.0.11 / Theme 6.5.1):** **Hero filter** runs on static front pages (no `in_the_loop`); **SVG replace** tolerates attribute order. **Mautic:** `LPNW_Mautic_Sync` fills **mautic_email_vip/pro/free** from template names; **dispatcher** stores **mautic_email_id** on successful Mautic send. **Email:** contact form **To** = `admin@` domain; **From** stays **hello@** (filters: `lpnw_admin_notify_mail_to_email`, `lpnw_admin_notify_mail_local_part`). **Parallax:** stronger layer motion + **scene fallback** when old markup has no `.lpnw-hero__parallax`. |
