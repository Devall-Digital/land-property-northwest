...
class LPNW_Dashboard {
	...
	public static function render_saved(): string {
		...
		$saved = $wpdb->get_results( $wpdb->prepare(
			"SELECT sp.*, p.address, p.postcode, p.price, p.source, p.property_type, p.application_type, p.source_url, p.description, p.raw_data, p.bedrooms, p.bathrooms, p.tenure_type, p.agent_name, p.first_listed_date, p.key_features_text
